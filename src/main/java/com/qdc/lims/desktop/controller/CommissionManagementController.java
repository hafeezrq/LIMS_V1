...
public class CommissionManagementController {
    ...
    private void setupTable() {
        ...
        billAmountColumn.setCellValueFactory(cellData -> {
            Double amount = cellData.getValue().getTotalBillAmount();
            return new SimpleDoubleProperty(amount != null ? amount : 0.0).asObject();
        });
        ...
        commissionRateColumn.setCellValueFactory(cellData -> {
            Double rate = cellData.getValue().getCommissionPercentage();
            return new SimpleDoubleProperty(rate != null ? rate : 0.0).asObject();
        });
        ...
        commissionAmountColumn.setCellValueFactory(cellData -> {
            Double amount = cellData.getValue().getCalculatedAmount();
            return new SimpleDoubleProperty(amount != null ? amount : 0.0).asObject();
        });
    ...
    private void updateStatistics() {
        try {
            // Total unpaid
            Double totalUnpaid = commissionRepository.getTotalCommissionByStatus("UNPAID");
            totalUnpaidLabel.setText(currencyFormat.format(totalUnpaid != null ? totalUnpaid : 0.0));

            // Total paid
            Double totalPaid = commissionRepository.getTotalCommissionByStatus("PAID");
            totalPaidLabel.setText(currencyFormat.format(totalPaid != null ? totalPaid : 0.0));
            ...
            Double thisMonth = commissionRepository.findByTransactionDateBetween(startOfMonth, endOfMonth)
                    .stream()
                    .mapToDouble(c -> c.getCalculatedAmount() != null ? c.getCalculatedAmount() : 0.0)
                    .sum();
    ...
    private void quickPayCommission(CommissionLedger commission) {
        ...
        confirmation.setContentText(
                "Doctor: " + commission.getDoctor().getName() + "\n" +
                        "Amount: " + currencyFormat.format(commission.getCalculatedAmount()) + "\n\n" +
    ...
    private void showCommissionDetails(CommissionLedger commission) {
        ...
        grid.add(
                new Label(currencyFormat
                        .format(commission.getTotalBillAmount() != null ? commission.getTotalBillAmount() : 0.0)),
                1, row++);

        grid.add(new Label("Commission Rate:"), 0, row);
        grid.add(new Label(percentFormat.format(
                commission.getCommissionPercentage() != null ? commission.getCommissionPercentage() : 0.0) + "%"), 1,
                row++);

        grid.add(new Label("Commission Amount:"), 0, row);
        Label amountLabel = new Label(currencyFormat
                .format(commission.getCalculatedAmount() != null ? commission.getCalculatedAmount() : 0.0));
